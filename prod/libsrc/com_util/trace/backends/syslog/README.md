...
通常の利用者は `com_util/trace/logger.h` を経由して使い、syslog 固有 API が必要な場合だけ `com_util/trace/syslog.h` を直接扱います。
...
- 通常は `com_util_logger_set_os_level()` と `com_util_logger_write()` 系から透過的に利用される
- `COM_UTIL_LOG_LEVEL_VERBOSE` と `COM_UTIL_LOG_LEVEL_DEBUG` はどちらも `LOG_DEBUG` に集約される
...
`com_util/trace/syslog.h` は、syslog sink を直接生成したい場合の lower layer です。  
...
- `syslog()` 自体にはタイムアウト制御手段がありません
  ...
  その懸念がある場合は、OS トレースを `COM_UTIL_LOG_LEVEL_NONE` にしてファイル backend を主経路にする運用を検討してください
