...
{
    trace_syslog_sink_t *handle = trace_syslog_sink_create("syslog_test", LOG_USER); // [手順] - syslog sink を初期化する。
...
TEST_F(trace_syslogTest, test_write_returns_zero)
{
    ...
    int result = trace_syslog_sink_write(handle, LOG_INFO, "test message"); // [手順] - INFO レベルで書き込む。
...
TEST_F(trace_syslogTest, test_null_arguments_are_safe)
{
    ...
    EXPECT_EQ((trace_syslog_sink_t *)NULL, trace_syslog_sink_create(NULL, LOG_USER)); // [確認_異常系] - NULL ident で create が失敗すること。
    EXPECT_EQ(0, trace_syslog_sink_write(NULL, LOG_INFO, "test message"));            // [確認_異常系] - NULL ハンドルが安全であること。
    EXPECT_EQ(0, trace_syslog_sink_write(handle, LOG_INFO, NULL));                    // [確認_異常系] - NULL message が安全であること。

    trace_syslog_sink_destroy(handle);
    trace_syslog_sink_destroy(NULL); // [手順] - NULL ハンドルで destroy を呼ぶ。
}
