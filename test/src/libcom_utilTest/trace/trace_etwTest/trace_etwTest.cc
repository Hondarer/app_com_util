...
{
    trace_etw_provider_t *handle = trace_etw_provider_create(s_test_provider); // [手順] - ETW provider を登録する。
...
TEST_F(trace_etwTest, test_write_returns_zero)
{
    ...
    int result = trace_etw_provider_write(handle, 4, NULL, "test message"); // [手順] - INFO レベルで書き込む。
...
TEST_F(trace_etwTest, test_null_arguments_are_safe)
{
    ...
    EXPECT_EQ((trace_etw_provider_t *)NULL, trace_etw_provider_create(NULL)); // [確認_異常系] - NULL provider_ref で create が失敗すること。
    EXPECT_EQ(0, trace_etw_provider_write(NULL, 4, NULL, "test message"));    // [確認_異常系] - NULL ハンドルが安全であること。
    EXPECT_EQ(0, trace_etw_provider_write(handle, 4, NULL, NULL));            // [確認_異常系] - NULL message が安全であること。

    trace_etw_provider_destroy(handle);
    trace_etw_provider_destroy(NULL); // [手順] - NULL ハンドルで destroy を呼ぶ。
}
